 ...
 * paths and assert each returns the documented bool + nora_spi_i2s_tdm_error_t
 ...
 * double-start) are exercised only in a NORA_TDM_USE_SPI2=1 build (logged skipped otherwise).
 * For contract-regression verification only; keep 0 for the shipped starter.
 *
 * Two on-board runs cover the matrix:
 *   Run A (single-leg): NEG=1, NORA_TDM_USE_SPI2=0, SMOKE=1 -> single-leg matrix, then the
 *          smoke resumes (proves the harness left the HAL clean).
 *   Run B (2-leg):      NEG=1, NORA_TDM_USE_SPI2=1, SMOKE=0 -> 2-leg matrix only. SMOKE must be
