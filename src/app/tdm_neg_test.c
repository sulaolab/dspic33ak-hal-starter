 ...
 * bool + nora_spi_i2s_tdm_error_t. Pure API-contract test: no external signals needed;
...
static bool neg_ready_hook( nora_spi_i2s_tdm_clock_role_t role )
{
    (void)role;
    return s_test_ready;
}
...
static const nora_spi_i2s_tdm_port_t s_neg_test_port =
{
    .configure_pins      = NULL,
...
static void neg_expect( const char *label, bool got,
                        bool want_ret, nora_spi_i2s_tdm_error_t want_err )
{
...
//===========================================================
static nora_spi_i2s_tdm_config_t neg_cfg( nora_spi_i2s_tdm_clock_role_t role, bool ckp )
{
    nora_spi_i2s_tdm_config_t c;
    c.format                        = NORA_SPI_I2S_TDM_FORMAT_TDM;
    c.clock_role                    = role;
    c.slots_per_fs                  = (uint8_t)NORA_TDM_SLOTS_PER_FS;
    c.word_bits                     = 32u;
    c.fs_shape                      = NORA_SPI_I2S_TDM_FS_PULSE;   // no CLC10 (self-test)
    c.block_frames                  = (uint16_t)NORA_TDM_BLOCK_FRAMES;
    c.brg                           = 7u;
    c.mclk_enable                   = true;
...
static void neg_null_handle( void )
{
    neg_expect( "inst_start(NULL) -> BAD_INSTANCE",
                nora_spi_i2s_tdm_inst_start( NULL ),
                false, NORA_SPI_I2S_TDM_ERR_BAD_INSTANCE );
    neg_expect( "inst_stop(NULL) -> BAD_INSTANCE",
                nora_spi_i2s_tdm_inst_stop( NULL ),
                false, NORA_SPI_I2S_TDM_ERR_BAD_INSTANCE );
}
...
static void neg_sync_domain_range( void )
{
    ...
    nora_spi_i2s_tdm_leg_setup_t setups[2];
    for( uint8_t i = 0u; i < n; i++ )
    {
        setups[i].stream      = neg_cfg( (i == 0u) ? NORA_SPI_I2S_TDM_CLOCK_MASTER
                                                   : NORA_SPI_I2S_TDM_CLOCK_SLAVE, true );
    ...
    neg_expect( "configure_system(sync_domain=32) -> TOPOLOGY",
                nora_spi_i2s_tdm_configure_system( setups, n ),
                false, NORA_SPI_I2S_TDM_ERR_TOPOLOGY );
}

// SINGLE-mode + primary-only per-leg API, and the open()/mode gates around it.
static void neg_single_mode( nora_spi_i2s_tdm_inst_t *spi1 )
{
    const nora_spi_i2s_tdm_config_t master = neg_cfg( NORA_SPI_I2S_TDM_CLOCK_MASTER, true );

    // inst_configure -> SINGLE mode.
    neg_expect( "inst_configure(primary) -> ok",
                nora_spi_i2s_tdm_inst_configure( spi1, &master ),
                true, NORA_SPI_I2S_TDM_ERR_NONE );

    // Start before open() -> NOT_OPEN (opened is checked before the readiness re-check).
    neg_expect( "inst_start before open() -> NOT_OPEN",
                nora_spi_i2s_tdm_inst_start( spi1 ),
                false, NORA_SPI_I2S_TDM_ERR_NOT_OPEN );

    // SYSTEM stop API in SINGLE mode -> CONFIG_MODE.
    neg_expect( "stop_domain(0) in SINGLE -> CONFIG_MODE",
                nora_spi_i2s_tdm_stop_domain( 0u ),
                false, NORA_SPI_I2S_TDM_ERR_CONFIG_MODE );
    neg_expect( "stop_all_domains in SINGLE -> CONFIG_MODE",
                nora_spi_i2s_tdm_stop_all_domains(),
                false, NORA_SPI_I2S_TDM_ERR_CONFIG_MODE );

    // Configure-while-open is rejected (open() consumes the committed config).
    neg_expect( "open() -> ok",
                nora_spi_i2s_tdm_open(),
                true, NORA_SPI_I2S_TDM_ERR_NONE );
    neg_expect( "inst_configure while open -> ALREADY_OPEN",
                nora_spi_i2s_tdm_inst_configure( spi1, &master ),
                false, NORA_SPI_I2S_TDM_ERR_ALREADY_OPEN );
    {
        ...
        setups[1].stream = neg_cfg( NORA_SPI_I2S_TDM_CLOCK_SLAVE, true ); setups[1].sync_domain = 0u;
        neg_expect( "configure_system while open -> ALREADY_OPEN",
                    nora_spi_i2s_tdm_configure_system( setups, nora_spi_i2s_tdm_instance_count() ),
                    false, NORA_SPI_I2S_TDM_ERR_ALREADY_OPEN );
    }
    ...
    neg_expect( "inst_start(primary) -> running",
                nora_spi_i2s_tdm_inst_start( spi1 ),
                true, NORA_SPI_I2S_TDM_ERR_NONE );
    neg_expect_cond( "is_running after start", nora_spi_i2s_tdm_is_running() );
    neg_expect( "close() while running -> ALREADY_RUNNING",
                nora_spi_i2s_tdm_close(),
                false, NORA_SPI_I2S_TDM_ERR_ALREADY_RUNNING );
    neg_expect_cond( "still running after refused close", nora_spi_i2s_tdm_is_running() );

    // Proper teardown: stop then close.
    neg_expect( "inst_stop(primary) -> ok",
                nora_spi_i2s_tdm_inst_stop( spi1 ),
                true, NORA_SPI_I2S_TDM_ERR_NONE );
...
static void neg_readiness( nora_spi_i2s_tdm_inst_t *spi1 )
{
    const nora_spi_i2s_tdm_config_t master = neg_cfg( NORA_SPI_I2S_TDM_CLOCK_MASTER, true );

    neg_expect( "set_port(test) -> ok",
                nora_spi_i2s_tdm_set_port( &s_neg_test_port ),
                true, NORA_SPI_I2S_TDM_ERR_NONE );
    (void)nora_spi_i2s_tdm_inst_configure( spi1, &master );   // SINGLE (already checked above)
    ...
    neg_expect( "inst_start with ready=false -> CLOCK_NOT_READY",
                nora_spi_i2s_tdm_inst_start( spi1 ),
                false, NORA_SPI_I2S_TDM_ERR_CLOCK_NOT_READY );
    ...
    neg_expect( "set_port(NULL) -> ok",
                nora_spi_i2s_tdm_set_port( NULL ),
                true, NORA_SPI_I2S_TDM_ERR_NONE );
}

// SYSTEM-mode API + the domain-stop invalid-domain contract. Leaves mode = SYSTEM.
static void neg_system_mode( nora_spi_i2s_tdm_inst_t *spi1 )
{
    const uint8_t n = nora_spi_i2s_tdm_instance_count();
    nora_spi_i2s_tdm_leg_setup_t setups[2];
    for( uint8_t i = 0u; i < n; i++ )
    ...
    {
        const nora_spi_i2s_tdm_config_t m = neg_cfg( NORA_SPI_I2S_TDM_CLOCK_MASTER, true );
        neg_expect( "inst_configure in SYSTEM -> CONFIG_MODE",
                    nora_spi_i2s_tdm_inst_configure( spi1, &m ),
                    false, NORA_SPI_I2S_TDM_ERR_CONFIG_MODE );
    }
    ...
    neg_expect( "stop_domain(31) no member -> BAD_INSTANCE",
                nora_spi_i2s_tdm_stop_domain( 31u ),
                false, NORA_SPI_I2S_TDM_ERR_BAD_INSTANCE );
    neg_expect( "stop_domain(255) >=32 -> BAD_INSTANCE",
                nora_spi_i2s_tdm_stop_domain( 255u ),
                false, NORA_SPI_I2S_TDM_ERR_BAD_INSTANCE );
...
// Runtime-gated on instance_count()>=2 (a NORA_TDM_USE_SPI2=1 build); logged skipped otherwise.
static void neg_two_leg( nora_spi_i2s_tdm_inst_t *spi1, nora_spi_i2s_tdm_inst_t *spi2 )
{
    nora_spi_i2s_tdm_leg_setup_t s[2];

    // (g) two clock MASTERs in one sync domain -> TOPOLOGY (preflight).
    s[0].stream = neg_cfg( NORA_SPI_I2S_TDM_CLOCK_MASTER, true ); s[0].sync_domain = 0u;
    s[1].stream = neg_cfg( NORA_SPI_I2S_TDM_CLOCK_MASTER, true ); s[1].sync_domain = 0u;
    neg_expect( "configure_system(2 masters/domain) -> TOPOLOGY",
                nora_spi_i2s_tdm_configure_system( s, 2u ),
                false, NORA_SPI_I2S_TDM_ERR_TOPOLOGY );
    ...
    // are rejected earlier by the per-leg envelope check, so they can't isolate the framing path.)
    {
        ...
        s[0].stream = neg_cfg( NORA_SPI_I2S_TDM_CLOCK_MASTER, true ); s[0].sync_domain = 0u;
        s[1].stream = neg_cfg( NORA_SPI_I2S_TDM_CLOCK_SLAVE,  true ); s[1].sync_domain = 0u;
        ...
        // CKP (bclk_idle_high)
        s[1].stream = neg_cfg( NORA_SPI_I2S_TDM_CLOCK_SLAVE, false ); s[1].sync_domain = 0u;  // s[0] CKP=true
        ...
        s[1].stream = neg_cfg( NORA_SPI_I2S_TDM_CLOCK_SLAVE, true ); s[1].sync_domain = 0u;
        s[1].stream.fs_shape = NORA_SPI_I2S_TDM_FS_50PCT;   // s[0] is FS_PULSE
    ...
    {
        nora_spi_i2s_tdm_leg_setup_t a1, a2, c1, c2;
        ...
        neg_expect( "configure_system A (valid) -> ok",
                    nora_spi_i2s_tdm_configure_system( s, 2u ),
                    true, NORA_SPI_I2S_TDM_ERR_NONE );
        bool ga1 = nora_spi_i2s_tdm_inst_get_setup( spi1, &a1 );
        bool ga2 = nora_spi_i2s_tdm_inst_get_setup( spi2, &a2 );
        ...
        bool gc1 = nora_spi_i2s_tdm_inst_get_setup( spi1, &c1 );
        bool gc2 = nora_spi_i2s_tdm_inst_get_setup( spi2, &c2 );
    ...
    // review; see plan.)
    s[0].stream = neg_cfg( NORA_SPI_I2S_TDM_CLOCK_MASTER, true ); s[0].sync_domain = 0u;
    s[1].stream = neg_cfg( NORA_SPI_I2S_TDM_CLOCK_MASTER, true ); s[1].sync_domain = 1u;
    ...
    neg_expect( "start_all_domains -> ok",
                ...
                true, NORA_SPI_I2S_TDM_ERR_NONE );
    {
        nora_spi_i2s_tdm_status_t st1, st2;
        bool r1 = nora_spi_i2s_tdm_inst_get_status( spi1, &st1, false );
        bool r2 = nora_spi_i2s_tdm_inst_get_status( spi2, &st2, false );
    ...
    neg_expect( "start_domain(0) on running -> idempotent true",
                nora_spi_i2s_tdm_start_domain( 0u ),
                true, NORA_SPI_I2S_TDM_ERR_NONE );
    {
        nora_spi_i2s_tdm_status_t st1, st2;
        (void)nora_spi_i2s_tdm_inst_get_status( spi1, &st1, false );
        (void)nora_spi_i2s_tdm_inst_get_status( spi2, &st2, false );
...
bool tdm_neg_test_run( void )
{
    ...
    nora_spi_i2s_tdm_inst_t *spi2 = nora_spi_i2s_tdm_spi2();
    const uint8_t n = nora_spi_i2s_tdm_instance_count();
    ...
    {
        printf(" [NEG] skip 2-leg cases (g/h/framing/transactional/non-destructive): needs NORA_TDM_USE_SPI2=1\n" );
    }
