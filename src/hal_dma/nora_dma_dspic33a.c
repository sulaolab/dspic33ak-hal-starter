 ...
 *
 * Uses nora_dma_dspic33a_reg.h for register bit masks/positions and generic SFR
...
#include "nora_dma.h"
#include "nora_dma_dspic33a_reg.h"
#include "nora_dma_dspic33a_fast.h"
...
#define NORA_DMA_DSPIC33A_CHANNEL_COUNT  (sizeof(s_dma_ch) / sizeof(s_dma_ch[0]))

static const nora_dma_ch_regs_t *nora_dma_regs(nora_dma_channel_t ch)
{
    if ((unsigned)ch >= NORA_DMA_DSPIC33A_CHANNEL_COUNT) {
...
static bool nora_dma_dspic33a_size_to_reg(nora_dma_size_t size, uint32_t *value)
{
    if (value == 0) {
        return false;
    }
...
static bool nora_dma_dspic33a_addr_mode_to_reg(nora_dma_addr_mode_t mode,
                                                uint32_t *value)
{
    if (value == 0) {
        return false;
...
static bool nora_dma_dspic33a_trmode_to_reg(nora_dma_trmode_t mode,
                                             uint32_t *value)
{
    if (value == 0) {
        return false;
...
static bool nora_dma_dspic33a_trigger_to_chsel(nora_dma_trigger_t trigger,
                                                uint32_t *chsel)
{
    if (chsel == 0) {
        return false;
...
void nora_dma_global_init(void)
{
    ...
    nora_dma_reg_set(&DMACON, NORA_DMA_DSPIC33A_CON_ON);  /* DMACONbits.ON = 1 */
    ...
    nora_dma_reg_set(&BMXINITPR, NORA_DMA_DSPIC33A_BMX_INITPR_DMAPR);

    DMAHIGH = NORA_DMA_DSPIC33A_ADDR_WINDOW_HIGH;
    DMALOW  = NORA_DMA_DSPIC33A_ADDR_WINDOW_LOW;
}
...
bool nora_dma_global_is_ready(void)
{
    ...
    if (!nora_dma_reg_is_set(&DMACON, NORA_DMA_DSPIC33A_CON_ON)) {
        return false;
    }
    if (DMAHIGH != NORA_DMA_DSPIC33A_ADDR_WINDOW_HIGH) {
        return false;
    }
    if (DMALOW != NORA_DMA_DSPIC33A_ADDR_WINDOW_LOW) {
        return false;
    }
    if (!nora_dma_reg_is_set(&BMXINITPR, NORA_DMA_DSPIC33A_BMX_INITPR_DMAPR)) {
        return false;
    }
    return true;
}
...
bool nora_dma_channel_config(nora_dma_channel_t ch, const nora_dma_channel_cfg_t *cfg)
{
    ...
    if (!nora_dma_dspic33a_size_to_reg(cfg->size, &size) ||
        !nora_dma_dspic33a_addr_mode_to_reg(cfg->src_mode, &src_mode) ||
        !nora_dma_dspic33a_addr_mode_to_reg(cfg->dst_mode, &dst_mode) ||
        !nora_dma_dspic33a_trmode_to_reg(cfg->tr_mode, &trmode) ||
        !nora_dma_dspic33a_trigger_to_chsel(cfg->trigger, &trigger)) {
        return false;
    }
    ...
    *r->CNT = cfg->count;
    ...
    nora_dma_reg_write_field(r->CH, NORA_DMA_DSPIC33A_CH_SAMODE_MASK,
                                  NORA_DMA_DSPIC33A_CH_SAMODE_POS, src_mode);
    nora_dma_reg_write_field(r->CH, NORA_DMA_DSPIC33A_CH_DAMODE_MASK,
                                  NORA_DMA_DSPIC33A_CH_DAMODE_POS, dst_mode);
    nora_dma_reg_write_field(r->CH, NORA_DMA_DSPIC33A_CH_SIZE_MASK,
                                  NORA_DMA_DSPIC33A_CH_SIZE_POS, size);
    nora_dma_reg_write_field(r->CH, NORA_DMA_DSPIC33A_CH_TRMODE_MASK,
                                  NORA_DMA_DSPIC33A_CH_TRMODE_POS, trmode);

    if (cfg->reload_count) { nora_dma_reg_set(r->CH, NORA_DMA_DSPIC33A_CH_RELOADC); }
    if (cfg->reload_src)   { nora_dma_reg_set(r->CH, NORA_DMA_DSPIC33A_CH_RELOADS); }
    if (cfg->reload_dst)   { nora_dma_reg_set(r->CH, NORA_DMA_DSPIC33A_CH_RELOADD); }

    if (cfg->half_int_en)  { nora_dma_reg_set(r->CH, NORA_DMA_DSPIC33A_CH_HALFEN); }
    if (cfg->done_int_en)  { nora_dma_reg_set(r->CH, NORA_DMA_DSPIC33A_CH_DONEEN); }

    /* Trigger source (DMAxSELbits.CHSEL). */
    nora_dma_reg_write_field(r->SEL, NORA_DMA_DSPIC33A_SEL_CHSEL_MASK,
                                  NORA_DMA_DSPIC33A_SEL_CHSEL_POS, trigger);
...
bool nora_dma_channel_enable(nora_dma_channel_t ch, bool enable)
{
    ...
    if (enable) {
        ...
        nora_dma_reg_set(r->CH, NORA_DMA_DSPIC33A_CH_CHEN);
    } else {
        /* Disable is always allowed (safe direction), even if not "ready". */
        nora_dma_reg_clear(r->CH, NORA_DMA_DSPIC33A_CH_CHEN);
    }
    return true;
}
...
nora_dma_half_t nora_dma_half_from_status(nora_dma_status_t status)
{
    ...
    if ((status & NORA_DMA_DSPIC33A_STAT_DONE) != 0u) {
        return NORA_DMA_HALF_SECOND;
    }
    if ((status & NORA_DMA_DSPIC33A_STAT_HALF) != 0u) {
