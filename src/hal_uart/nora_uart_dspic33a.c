...
#include "nora_uart.h"
#include "nora_uart_dspic33a_rx_isr_ring.h"
#include "nora_uart_dspic33a_device.h"
#include "nora_uart_dspic33a_reg.h"
...
static nora_uart_status_t uart_get_regs(
    ...
    const nora_uart_dspic33a_regs_t **regs);
...
{
    const nora_uart_dspic33a_regs_t *r;
    ...
    *r->STAT = 0u;
    ...
    nora_uart_dspic33a_reg_set(r->CON, NORA_UART_DSPIC33A_CON_CLKSEL);
    nora_uart_dspic33a_reg_set(r->CON, NORA_UART_DSPIC33A_CON_CLKMOD);

    *r->BRG = brg;

    /* TX FIFO write enable. */
    nora_uart_dspic33a_reg_set(r->STAT, NORA_UART_DSPIC33A_STAT_TXWRE);

    if (config->enable_tx) {
        nora_uart_dspic33a_reg_set(r->CON, NORA_UART_DSPIC33A_CON_TXEN);
    }
    if (config->enable_rx) {
        nora_uart_dspic33a_reg_set(r->CON, NORA_UART_DSPIC33A_CON_RXEN);
    }

    /* Enable the module last. */
    nora_uart_dspic33a_reg_set(r->CON, NORA_UART_DSPIC33A_CON_ON);
     ...
     * RX ring now. nora_uart_dspic33a_rx_isr_config() requires the instance to be
    ...
    if (config->rx_mode == NORA_UART_RX_MODE_ISR_RING) {
        nora_uart_dspic33a_rx_isr_config_t rx_cfg;
        ...
        st = nora_uart_dspic33a_rx_isr_config(inst, &rx_cfg);
        if (st != NORA_UART_OK) {
            (void)nora_uart_deinit(inst);
            return st;
        }

        st = nora_uart_dspic33a_rx_isr_enable(inst);
        if (st != NORA_UART_OK) {
            (void)nora_uart_deinit(inst);
            return st;
        }
    ...
    nora_uart_instance_t inst)
{
    ...
    if (g_rx_mode[inst] == NORA_UART_RX_MODE_ISR_RING) {
        (void)nora_uart_dspic33a_rx_isr_disable(inst);
    }
    ...
    nora_uart_dspic33a_reg_clear(r->CON,
                             NORA_UART_DSPIC33A_CON_TXEN |
                             NORA_UART_DSPIC33A_CON_RXEN |
                             NORA_UART_DSPIC33A_CON_ON);
...
{
    return nora_uart_dspic33a_instance_is_present(inst);
}
...
bool nora_uart_rx_ready(nora_uart_instance_t inst)
{
    ...
    if (g_rx_mode[inst] == NORA_UART_RX_MODE_ISR_RING) {
        return nora_uart_dspic33a_rx_isr_ready(inst);
    }

    /* Polling backend: RX has data when the RX FIFO is NOT empty. */
    return !nora_uart_dspic33a_reg_is_set(r->STAT, NORA_UART_DSPIC33A_STAT_RXBE);
}
...
bool nora_uart_tx_ready(nora_uart_instance_t inst)
{
    ...
    return !nora_uart_dspic33a_reg_is_set(r->STAT, NORA_UART_DSPIC33A_STAT_TXBF);
}
...
bool nora_uart_tx_done(nora_uart_instance_t inst)
{
    ...
    return nora_uart_dspic33a_reg_is_set(r->STAT, NORA_UART_DSPIC33A_STAT_TXMTIF) &&
           nora_uart_dspic33a_reg_is_set(r->STAT, NORA_UART_DSPIC33A_STAT_TXBE);
}
    ...
    uint8_t data)
{
    ...
    while (nora_uart_dspic33a_reg_is_set(r->STAT, NORA_UART_DSPIC33A_STAT_TXBF)) {
    ...
    uint8_t *data)
{
    ...
    if (g_rx_mode[inst] == NORA_UART_RX_MODE_ISR_RING) {
        return nora_uart_dspic33a_rx_isr_read_byte(inst, data);
    }

    /* Polling backend: read directly from the RX FIFO. */
    if (nora_uart_dspic33a_reg_is_set(r->STAT, NORA_UART_DSPIC33A_STAT_RXBE)) {
...
void nora_uart_rx_flush(nora_uart_instance_t inst)
{
    ...
    if (g_rx_mode[inst] == NORA_UART_RX_MODE_ISR_RING) {
        nora_uart_dspic33a_rx_isr_flush(inst);
        return;
    }

    /* Polling backend: drain the hardware RX FIFO and clear the overflow flag. */
    while (!nora_uart_dspic33a_reg_is_set(r->STAT, NORA_UART_DSPIC33A_STAT_RXBE)) {
        (void)(*r->RXB);
    }

    if (nora_uart_dspic33a_reg_is_set(r->STAT, NORA_UART_DSPIC33A_STAT_RXFOIF)) {
        nora_uart_dspic33a_reg_clear(r->STAT, NORA_UART_DSPIC33A_STAT_RXFOIF);
    }
}
    ...
    nora_uart_rx_status_t *status)
{
    ...
    if (g_rx_mode[inst] == NORA_UART_RX_MODE_ISR_RING) {
        nora_uart_dspic33a_rx_isr_status_t isr_status;

        nora_uart_dspic33a_rx_isr_status_get(inst, &isr_status);
    ...
    nora_uart_instance_t inst)
{
    ...
    if (g_rx_mode[inst] == NORA_UART_RX_MODE_ISR_RING) {
        nora_uart_dspic33a_rx_isr_status_clear(inst);
    }
    ...
    void *user_data)
{
    ...
    if (!nora_uart_dspic33a_instance_is_present(inst)) {
    ...
    bool enable)
{
    ...
    if (enable) {
        ...
        nora_uart_dspic33a_reg_clear(r->CON, NORA_UART_DSPIC33A_CON_TXEN);
    }
    ...
    bool enable)
{
    ...
    if (enable) {
        ...
        nora_uart_dspic33a_reg_clear(r->CON, NORA_UART_DSPIC33A_CON_RXEN);
    }
    ...
    uint32_t baudrate)
{
    ...
    if (!nora_uart_dspic33a_reg_is_set(r->STAT, NORA_UART_DSPIC33A_STAT_TXMTIF) ||
        !nora_uart_dspic33a_reg_is_set(r->STAT, NORA_UART_DSPIC33A_STAT_TXBE)) {
        return NORA_UART_ERR_BUSY;
    }
    ...
    size_t length)
{
     ...
     * the ISR ring backend; nora_uart_dspic33a_async_rx_feed() picks the bytes up as
...
void nora_uart_tx_irq_handler(nora_uart_instance_t inst)
{
    ...
    while ((g_tx_count[inst] < g_tx_len[inst]) &&
           !nora_uart_dspic33a_reg_is_set(r->STAT, NORA_UART_DSPIC33A_STAT_TXBF)) {
...
/* -------------------------------------------------------------------------- */
/* nora_uart_dspic33a_async_rx_feed (internal ISR hook)                           */
/* -------------------------------------------------------------------------- */
bool nora_uart_dspic33a_async_rx_feed(nora_uart_instance_t inst, uint8_t byte)
{
...
/* -------------------------------------------------------------------------- */
/* nora_uart_dspic33a_async_notify_events (internal ISR hook)                     */
/* -------------------------------------------------------------------------- */
void nora_uart_dspic33a_async_notify_events(
    nora_uart_instance_t inst,
    uint32_t events)
{
...
static nora_uart_status_t uart_get_regs(
    ...
    const nora_uart_dspic33a_regs_t **regs)
{
    const nora_uart_dspic33a_device_t *dev;
    ...
    dev = nora_uart_dspic33a_get_device(inst);
    if (dev == 0) {
        return NORA_UART_ERR_NOT_PRESENT;
    }
...
static bool uart_tx_irq_set_priority(nora_uart_instance_t inst, uint8_t prio)
{
    return nora_uart_dspic33a_device_set_tx_irq_priority(inst, prio);
}
...
static bool uart_tx_irq_clear_flag(nora_uart_instance_t inst)
{
    return nora_uart_dspic33a_device_tx_irq_clear_flag(inst);
}
...
static bool uart_tx_irq_raise_flag(nora_uart_instance_t inst)
{
    return nora_uart_dspic33a_device_tx_irq_raise_flag(inst);
}
...
static bool uart_tx_irq_enable(nora_uart_instance_t inst, bool enable)
{
    return nora_uart_dspic33a_device_tx_irq_enable(inst, enable);
}
...
static bool uart_rx_irq_enable(nora_uart_instance_t inst, bool enable)
{
    return nora_uart_dspic33a_device_rx_irq_enable(inst, enable);
}
...
static uint8_t uart_rx_irq_get_enable(nora_uart_instance_t inst)
{
    ...
    if (!nora_uart_dspic33a_device_rx_irq_is_enabled(inst, &enabled)) {
        return 0u;
    }
